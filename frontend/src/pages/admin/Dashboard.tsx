  ...
  export default function Dashboard() {
    ...
    // ... (Mantener filtros e inicio de ticketsFiltrados arriba de forma normal)

    // 1. Corrección del error de la línea 151 (hasOwnProperty seguro)
    // Reubicamos 'ticketsResueltosPorSemana' fuera de cualquier otra lógica al nivel de raíz
    const ticketsResueltosPorSemana = useMemo(() => {
      ...
      ticketsFiltrados.forEach((ticket) => {
        if (ticket.estado === 'cerrado' && ticket.fechaResolucion) {
          ...
          inicioSemana.setDate(inicioSemana.getDate() - inicioSemana.getDay());
          const key = inicioSemana.toISOString().split('T')[0];
          // Cambio seguro para evitar el error de ESLint de hasOwnProperty
          if (Object.prototype.hasOwnProperty.call(semanas, key)) {
            semanas[key]++;
          }
        }
      });
    ...
    // 2. Corrección del error de la línea 130 y TS2367 (Métricas generales sin hooks internos)
    const metrics = useMemo(() => {
      ...
      const tiempoPromedioRespuesta = ticketsConAsignacion.length > 0
          ? ticketsConAsignacion.reduce((acc, t) => acc + new Date(t.fechaCreacion).getTime(), 0) / ticketsConAsignacion.length
          : 0;
      ...
      const tiempoPromedioResolucion = ticketsCerrados.length > 0
          ? ticketsCerrados.reduce((acc, t) => acc + ((t.fechaResolucion ? new Date(t.fechaResolucion).getTime() : 0) - new Date(t.fechaCreacion).getTime()), 0) / ticketsCerrados.length
          : 0;

      // Corrección TS2367: Forzamos la comparación de rol en minúsculas ignorando rigideces del Enum
      const tecnicos = users.filter((u) => String(u.rol).toLowerCase() === 'tecnico' || String(u.rol).toLowerCase() === 'admin');
      ...
      return {
        ...
        ticketsPorCategoria,
        ticketsPorEdificio,
        ticketsResueltosPorSemana // Inyectamos el resultado calculado limpiamente arriba
      };
    }, [ticketsFiltrados, users, ticketsResueltosPorSemana]); // Añadimos como dependencia del hook principal
        ...
        <Paper sx={{ p: 3, mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
            ...
            <FormControl sx={{ flex: { xs: '1 1 100%', sm: '1 1 30%' } }}>
              ...
              >
                <MenuItem value="">Todos</MenuItem>
                {users.filter((u) => String(u.rol) === 'tecnico' || String(u.rol) === 'admin').map((u) => (
